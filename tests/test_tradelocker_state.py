...
class MissingBalanceTests(unittest.TestCase):
    ...
    @patch.object(ts.tl, "get_order_history", lambda config, limit=50: [])
    @patch.object(ts.tl, "get_positions", lambda config: [])
    @patch.object(ts.tl, "get_account_state", lambda config: {})
    def test_missing_balance_raises_incomplete_data(self):
        with self.assertRaises(ts.IncompleteAccountData):
            ts.build_account(self._FakeConfig())

    @patch.object(ts.tl, "get_order_history", lambda config, limit=50: [])
    @patch.object(ts.tl, "get_positions", lambda config: [])
    @patch.object(ts.tl, "get_account_state", lambda config: {"balance": "not-a-number"})
    ...
    @patch.object(ts.tl, "get_order_history", lambda config, limit=50: [])
    @patch.object(ts.tl, "get_positions", lambda config: [])
    @patch.object(ts.tl, "get_account_state", lambda config: {"balance": 5000.0})
    def test_valid_balance_is_used_directly(self):
        account = ts.build_account(self._FakeConfig())
