...
def build_account(config: tl.TradeLockerConfig) -> dict[str, Any]:
    ...
    balance = _num_or_none(state.get("balance"))
    if balance is None:
...
def build_open_trade(config: tl.TradeLockerConfig) -> dict[str, Any] | None:
    ...
    return {
        ...
        # prices, so a second lookup against /orders is required.
        "stopLoss": 0.0,
        "takeProfit": 0.0,
        "riskAmount": None,
        "quantity": quantity,
...
def build_trades(config: tl.TradeLockerConfig, limit: int = 50) -> list[dict[str, Any]]:
    names = _instrument_name_map(config)
    history = tl.get_order_history(config, limit=limit * 2)  # extra headroom pre-grouping
    positions = tl.get_positions(config)
    closed_trades = _group_closed_trades_by_position(history)
...
def current_state(config: tl.TradeLockerConfig) -> dict[str, Any]:
    account = build_account(config)  # raises IncompleteAccountData if balance is unusable
