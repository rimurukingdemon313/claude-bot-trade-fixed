...
async function executeSymbolCycle(symbol: TradedSymbol): Promise<{
  ...
  if (hasCompleteOrderPlan) {
    try {
      ...
      // submitting, not just once at the top of this function. The AI
      // call and risk-engine call above can each take seconds; without
      // this second check here, a position opened by a concurrent cycle
      // (e.g. a manual trigger overlapping with the scheduler) during
      // that window would go undetected and a duplicate order could be
      // submitted for the same symbol.
